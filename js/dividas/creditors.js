...
export function renderCreditors() {
  ...
  const list = $('creditorsList');
  if (!list) return;
  if (!state.creditors.length) {
    list.innerHTML = emptyCard('Nenhum credor cadastrado', 'Cadastre credores para usar na criação das dívidas.');
  } else {
    list.innerHTML =
...
window.saveCreditor = async function() {
  ...
  if (state.editingCreditorId) {
    await updateDoc(debtCreditorDoc(state.editingCreditorId), payload);
    ...
    const created = await addDoc(debtCreditorsColl(), { ...payload, createdAt: serverTimestamp() });
...
window.handleCreditorLogoUpload = function(event) {
  ...
  if (file.size > 250000) { event.target.value = ''; return showToast('Use uma logo menor, até 250 KB.'); }
...
window.openUnifyCreditorModal = function(creditorId) {
  ...
  const sourceEl = $('unifySourceName');
  const targetEl = $('unifyTargetSelect');
  if (sourceEl) sourceEl.textContent = creditor.name;
  if (targetEl) {
    targetEl.innerHTML = orcCreditors.length
      ? orcCreditors.map(c => '<option value="' + c.id + '">' + escapeHtml(c.name) + '</option>').join('')
...
window.confirmUnifyCreditor = async function() {
  ...
  const debtsToUpdate = state.debts.filter(d => d.creditorId === sourceId);
  if (debtsToUpdate.length) {
    const batch = writeBatch();
    debtsToUpdate.forEach(d => {
      batch.update(debtDoc(d.id), { creditorId: targetId, updatedAt: serverTimestamp() });
      d.creditorId = targetId;
    });
    await batch.commit();
  }

  await deleteDoc(debtCreditorDoc(sourceId));
  state.creditors = state.creditors.filter(c => c.id !== sourceId);
  state.unifyingCreditorId = null;
  document.getElementById('divUnifyCreditorDialog').close();
  if (state.renderFn) state.renderFn();
